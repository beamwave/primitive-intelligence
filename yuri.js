...
const checkSentenceFor = (comment, params) => {
  ...
  if (keys.includes('mustHave')) {
    const regex = new RegExp($`${params.mustHave}`, flags)
    if (!comment.match(regex)) return false
  }
  if (keys.includes('cantHave')) {
    const regex = new RegExp($`${params.cantHave}`, flags)
    if (comment.match(regex)) return false
  }
  return true
}
...
const respondToComment = (subjectivity, polarity, score) => {
  ...
  if (
    ...
    if (currentUserInfoInMemory.accessLevel.match(/(1|2)/)) {
      ...
      const text = `${mentionedUserInfoInMemory.username}'s first comment was "${mentionedUserInfoInMemory.firstComment}"`
        executeWithErrorHandling(text => writeToChat(text))
      }
